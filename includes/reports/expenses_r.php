...
$typeQury = " ";
if($_GET['type'] !== null and $_GET['type'] !== ''){
	$type = $_GET['type'];
	$typeQury = " type = " . $type . " and "; 
} 
?>
                                                <?php
...
###########################################
	...
	*/
$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_expenses where type = ".$type." and left(date,10) BETWEEN '".$from."' AND '".$to."' order by $orderby ";
	...
	$sql = "SELECT * FROM ".$prefix."_expenses where ". $typeQury ." left(date,10) BETWEEN '".$from."' AND '".$to."' order by $orderby LIMIT $start, $limit";			
...
$result_get = mysqli_query($con,"SELECT Amount FROM ".$prefix."_expenses where ".$typeQury."  left(date,10) BETWEEN '".$from."' AND '".$to."'");
