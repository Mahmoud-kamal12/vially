...
if($_GET['del']!==null){
    if($user_IsAdmin==1){
 if(mysqli_query($con,"DELETE FROM ".$prefix."_sales_inv WHERE inv_id='".$_GET['del']."'")){
...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	...
	*/
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_sales_inv where ".$seller." type in('1') and  left(date, 10) BETWEEN '".$from."' AND '".$to."'";
...
$sql = "SELECT * FROM ".$prefix."_sales_inv where ".$seller." type in('1') and left(date,10) BETWEEN '".$from."' AND '".$to."' order by $orderby $type LIMIT $start, $limit";
...
<img src="images/edit.png"/><?php */?>
 ...
 <a href="#" onclick="javascript:void window.open('edit_invoice.php?id=<?php echo"".$row['inv_id'].""; ?>&type=sale','1390937502641','width=700,height=600,toolbar=0,menubar=0,location=0,status=0,scrollbars=1,resizable=0,left=0,top=0,menubar=0');return false;"><img src="images/edit.png"/></a>
<a href="#" onclick="javascript:void window.open('invoice.php?id=<?php echo"".$row['inv_id'].""; ?>&type=sale','1390937502641','width=700,height=600,toolbar=0,menubar=0,location=0,status=0,scrollbars=1,resizable=0,left=0,top=0,menubar=0');return false;"><img src="images/view.png"/></a>
...
$result_get = mysqli_query($con,"SELECT Total,discount FROM ".$prefix."_sales_inv where ".$seller." type='1' and left(date,10) BETWEEN '".$from."' AND '".$to."'");
if(mysqli_num_rows($result_get)>0){
while($row_get = mysqli_fetch_array($result_get))
  {
	 $total+=($row_get['Total']-$row_get['discount']);
  }
}
echo $total;
 ?></th>
