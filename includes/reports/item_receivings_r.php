...
<thead>
<td colspan="7" style="background-color:#09F;"><strong style="color:#FFF; font-size:22px;"><?php echo"$Purchases_items_lang"; ?>/ <?php echo"".$_GET['item'].""; ?> <?php echo"$from_lang"; ?> <?php echo"".$_GET['from'].""; ?> <?php echo"$to_lang"; ?> <?php echo"".$_GET['to'].""; ?></strong></td>
</thead>
...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	...
	*/
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_receivings where item=".$_GET['q']." and type='1' and  left(date, 10) BETWEEN '$from' AND '$to'";
	$total_pages = @mysqli_fetch_array(mysqli_query($con,$query));
	$total_pages = $total_pages[num];
		
	/* Setup vars for query. */
	$targetpage = "?q=".$_GET['q']."&from=".$_GET['from']."&to=".$_GET['to']."&limit=".$_GET['limit']."&orderby=".$_GET['orderby']."&type=".$_GET['type']."&reports=suppliers_receivings"; 	//your file name  (the name of this file)
...
$sql = "SELECT * FROM ".$prefix."_receivings where item=".$_GET['q']." and type='1' and left(date,10) BETWEEN '$from' AND '$to' order by date DESC LIMIT $start, $limit";	
	//}
...
$result_suppliers = mysqli_query($con,"SELECT id,name FROM ".$prefix."_suppliers WHERE id=".$row['SupplierID']."");
