...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	...
	*/
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_receivings_inv where  where Total<0 and type !='3' and  left(date, 10) BETWEEN '".$from."' AND '".$to."'";
...
$sql = "SELECT * FROM ".$prefix."_receivings_inv where Total<0 and type !='3' and left(date,10) BETWEEN '".$from."' AND '".$to."' order by $orderby $type LIMIT $start, $limit";	
...
?>
 ...
 <a href="#" onclick="javascript:void window.open('edit_invoice.php?id=<?php echo"".$row['inv_id'].""; ?>&type=receivings_returns','1390937502641','width=700,height=600,toolbar=0,menubar=0,location=0,status=0,scrollbars=1,resizable=0,left=0,top=0,menubar=0');return false;"><img src="images/edit.png"/></a>
...
$result_get = mysqli_query($con,"SELECT Total FROM ".$prefix."_receivings_inv where  Total<0 and type !='3' and left(date,10) BETWEEN '".$from."' AND '".$to."'");
