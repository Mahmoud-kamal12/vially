...
$Totalnext=$totalgetbalance+$Totallast;
  ...
  <?php
$result_get = mysqli_query($con,"SELECT * FROM ".$prefix."_sales_inv where supplier='".$_GET['id']."' and left (date,10) BETWEEN '$from' AND '$to' order by date ASC,id ASC");
...
while($row_item=mysqli_fetch_array($result_item))
 ...
 echo' <tr>
    ...
    <td>'.$row_getinv['Price'].'</td>
                <td>'.get_color_name($row_getinv['color']).'</td>
            <td>'.Get_size_name($row_getinv['size']).'</td>
    <td>'.$Quantitys.'</td>
    <td>'.$item_name.'</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>';
