...
if (isset($_GET['unsuspend'])) {
    if (mysqli_query($con, "INSERT INTO " . $prefix . "_conversion_temporary(item, Price, Quantity, Discount, Total, BuyPrice, date, type, sales_type, subqty, size, color, user_id)
SELECT item,Price,Quantity,Discount,Total,BuyPrice,date,type,sales_type,subqty,size,color,user_id from " . $prefix . "_sales_suspended")) {
...
####################################
mysqli_query($con, "UPDATE ".$prefix."_conversion_temporary SET type='1' where user_id='$user_id' and type='2'");
mysqli_query($con, "UPDATE ".$prefix."_conversion_temporary SET Quantity=Quantity*-1 where user_id='$user_id' and Quantity<=0");
mysqli_query($con, "UPDATE ".$prefix."_conversion_temporary SET Total=Total*-1 where user_id='$user_id' and Total<=0");

if (isset($_POST['date'])) {
    $_POST['date'] = str_replace("/", "-", $_POST['date']);
    $DueDate = Trim(date('Y-m-d', strtotime($_POST['date'])));
    ##################
    if (mysqli_query($con, "UPDATE " . $prefix . "_conversion_temporary SET  date='$DueDate' where user_id=$user_id")) {
                            ...
                            <div>
                                ...
                                <div style="width:100%; padding-top:10px; text-align:center; margin:0 auto;border:0px solid #CCC; border-radius:5px;"">
                                    ...
                                    if (isset($_GET['suspend'])) {
                                        if (mysqli_query($con, "INSERT INTO " . $prefix . "_sales_suspended(item, Price, Quantity, Discount, Total, BuyPrice, date, type, sales_type, subqty, size, color, user_id )
SELECT item,Price,Quantity,Discount,Total,BuyPrice,date,type,sales_type,subqty,size,color,user_id from " . $prefix . "_conversion_temporary")) {
                                            if (mysqli_query($con, "DELETE FROM " . $prefix . "_conversion_temporary where user_id='$user_id'")) {
                                    ...
                                    ?>
                                    <a href="swap.php?q=d"><img src="images/c.png" style="width:30px; height:30px; float:left; border:0px;"  title="<?php echo"$Cancel_lang"; ?>"/></a>
                                    ...
                                    if ($Retail_allow == "1") {
                                        if ($get_db_sales_type == "1") {
                                            echo' <a href="swap.php?sales_type=2"><img src="images/Shopping_retail-04-128.png" style="width:30px; height:30px; float:left; border:0px;" title="'.$Retail_lang.'"   /></a>';
                                        } else if ($get_db_sales_type == "2") {
                                            echo'<a href="swap.php?sales_type=1"><img src="images/g.png" style="width:30px; height:30px; float:left; border:0px;" title="'.$Wholesaling_lang.'" /></a>';
                                        } else {
                                            echo'<a href="swap.php?sales_type=1"><img src="images/g.png" style="width:30px; height:30px; float:left; border:0px;" title="'.$Wholesaling_lang.'" /></a>';
                                        }
                                    }
                                    ...
                                    if ($_GET['print_inv'] !== null) {
                                        ##############
                                        $result_Get_supplier = mysqli_query($con, "SELECT  supplier  FROM ".$prefix."_conversions_inv where  inv_id=".$_GET['print_inv']."");
        ...
        <?php } ?>
							...
							'.$suspended_bill_lang.'
                                    ...
                                    if (is_numeric($_POST['submit']) == 1 and $_POST['submit'] != null) {
                                        ...
                                        ###################################
                                        $arrErrors = [];
                                        $result_upt = mysqli_query($con, "SELECT * FROM " . $prefix . "_conversion_temporary where user_id='$user_id' order by id DESC");
                                        ...
                                        if ($arrErrors == []) {
                                            ...
                                            if (false) {
                                                ...
                                                if ($paid < 0 or $paid == null or $paid == "") {
                                                    ...
                                                    $sqlpayments_suppliers = "INSERT INTO " . $prefix . "_conversions_inv ( Total, date, PaymentMethod, DueDate, notes, type, discount  ) VALUES ('" . ($paid * -1) . "','" . $DueDate . "','" . $pay . "','','" . $notes . "','3','" . $alldiscount . "')";
                                                    if (!mysqli_query($con, $sqlpayments_suppliers)) {
                                                        ...
                                                        if ($alldiscount <= 0) {
                                                            ...
                                                            $sqlpayments = "INSERT INTO " . $prefix . "_conversions_inv ( Total, date, PaymentMethod, DueDate, notes, type, discount  ) VALUES ('" . (($alldiscount) * -1) . "')";
...
######################
                                                ######################
                                                $result_up = mysqli_query($con, "SELECT * FROM " . $prefix . "_conversion_temporary where user_id='$user_id' order by id DESC");
                                                ...
                                                while ($row_up = mysqli_fetch_array($result_up)) {
                                                    ...
                                                    if (mysqli_query($con, "UPDATE " . $prefix . "_conversion_temporary SET inv_id='" . $get_LastInvoiceNo . "',Quantity='" . $quantity . "',
...
###########################################
                                                ...
                                                if (mysqli_query($con, "INSERT INTO " . $prefix . "_conversions_inv(inv_id, date, Total, PaymentMethod, DueDate, CheckNumber, notes, type, paid, discount  )
VALUES ('" . $get_LastInvoiceNo . "','" . $DueDate . "','" . $inv_Total . "','" . $pay . "','','" . $CheckNumber . "','" . $notes . "','1','" . $paid . "','" . $alldiscount . "')")) {
                                                    $Report_Create_Invoice = 1;
##############
                                                    if (mysqli_query($con, "INSERT INTO " . $prefix . "_conversions(Quantity, Discount, Total, BuyPrice, date, type, sales_type, subqty, inv_id, item, Price, size, color, user_id)
SELECT (CASE when sales_type=2 then  Quantity/subqty else Quantity END) ,Discount,Total,BuyPrice,date,type,sales_type,subqty,inv_id,item,Price,size,color,user_id from " . $prefix . "_conversion_temporary where user_id='$user_id'")) {
...
################
                                                ################################
                                                if (mysqli_query($con, "DELETE FROM " . $prefix . "_conversion_temporary where user_id='$user_id'")) {
                                                    header("refresh:1;url=swap.php?print_inv=" . $get_LastInvoiceNo . "");
                                                }
                                                //echo $inv_Total;
                                                ######################
                                            }
                                            ...
                                            foreach ($arrErrors as $e){
                                                ...
                                                <span style="float:left; padding-left:20px;"><img src="images/close.png" style="border:0px;" /></span> ' .
                                                    $e_item[0] . ' - مقاس ' . $e_size[0] . ' - ' . $e_color[0] . '  ( غير موجوده فى المخزن )   '
                                                    .'</div>';
                                    ...
                                    if ($_GET['del'] !== null) {
                                        if (mysqli_query($con, "DELETE FROM " . $prefix . "_conversion_temporary where user_id='$user_id' and id=" . $_GET['del'] . "")) {
                                            echo '<div style="text-align:center; background-color:#E18C80; border-radius:5px;">
							<span style="float:left; padding-left:20px;"><img src="images/chmarkicon.png" style="border:0px;" /></span>
							'.$Item_deleted_lang.'
							</div>';
                                    ...
                                    if (isset($_POST['submit'])) {
                                        ...
                                        while ($row_up = mysqli_fetch_array($result_up)) {
                                            ...
                                            if ($quantity != 0 or $quantity != "") {
                                                mysqli_query($con, "UPDATE " . $prefix . "_conversion_temporary SET  Quantity='" . $quantity . "',Price='" . $Price . "',Discount='" . $Discount . "',
                                            ...
                                            //	header( "refresh:0;url=swap.php" );
                                                ...
                                                if (@mysqli_num_rows($result_new) >= 1) {
                                                    ...
                                                    while ($row_new = mysqli_fetch_array($result_new)) {
                                                        ...
                                                        $sql = "INSERT INTO " . $prefix . "_conversion_temporary (item, Price, Quantity, Discount, Total, BuyPrice, date, type, subqty, sales_type, size ,color, user_id)
							...
							'.$no_stock_lang.'
                                                ...
                                                if (@mysqli_num_rows($result_new) >= 1) {
                                                    ...
                                                    while ($row_new = mysqli_fetch_array($result_new)) {
                                                        ...
                                                        $sql = "INSERT INTO " . $prefix . "_conversion_temporary (item, Price, Quantity, Discount, Total, type, date, BuyPrice, subqty, sales_type, size, color, user_id)
...
<form method="post">
                                        ...
                                        <table  border="1" style="font-size:16px; width:100%;  direction:rtl; border:1px; border-collapse:collapse; margin-top:10px; text-align:center;">
                                           ...
                                           <?php if($use_sizes==1){echo'<th class="text-center">'.$the_Size_lang.'</th>';} ?>
                                            ...
                                            <tbody>
                                                <?php
                                                $tbl_name = "" . $prefix . "_conversion_temporary";
                                                //your table name
                                                 ...
                                                 */
                                                $query = "SELECT COUNT(*) as num  FROM  " . $prefix . "_conversion_temporary where user_id='$user_id' order by id DESC";
                                                ...
                                                /* Get data. */
                                                $sql = "SELECT * FROM " . $prefix . "_conversion_temporary where user_id='$user_id' order by id DESC";
                ...
                while ($row_colors = mysqli_fetch_array($result_colors)) {
                    if ($row['color'] == $row_colors['id']) {
                        ...
                        echo '<option value="' . $row_colors['id'] . '">' . $row_colors['color'] . '</option >';
							...
							<td valign="middle"><a href="swap.php?del=' . $row['id'] . '"><img src="images/erase.png" style="border:0px;" /></a></td>
							</tr>';
                                                $i++;
                                            }
                                            ?>
        ...
        function processForm( e ){
            $.ajax({
                ...
                success: function( data, textStatus, jQxhr ){
                    $('#response').html( data );
                          var $container = $("#data");
        $container.load('multi_items_swap.php');
                },
